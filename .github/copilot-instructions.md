...
- Read `design/DEPLOYMENT_CONTEXT.md` before touching deployment, Docker, or anything under `deploy/`. It is the self-contained source of truth for how this app is containerised and run; `design/DOCKER_DEPLOYMENT_PLAN.md` holds the original design rationale and `docs/DEPLOYMENT.md` is the operator guide. The stacks are implemented and verified locally but nothing is deployed to a server yet, and the `durga` edge is untouched.
