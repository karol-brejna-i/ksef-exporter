...
- **[`DOCKER_DEPLOYMENT_PLAN.md`](./DOCKER_DEPLOYMENT_PLAN.md)** — implemented and verified locally (2026-09-13), **not yet deployed to a server**. Containerises both tenants as isolated Compose stacks (`ksef-parkowa` on port 8081, `ksef-portowa` on 8082, plus a throwaway `ksef-local` on 8091), each with an unpublished Fastify `api` container and an nginx `web` sidecar that serves the built SPA and proxies `/api` to it. Adds a multi-stage `Dockerfile` (targets `api`/`web`), a single parameterised `deploy/docker-compose.yml` driven by per-tenant env files, `deploy/ksef-stack.sh` as the sole operator entry point, backup/restore/smoke scripts, an unauthenticated `GET /health` route, and a `VACUUM INTO` snapshot tool (`pnpm run backup:db`). Verified end to end on a local throwaway stack: health, SPA, login, and an authenticated call through the sidecar, plus a real destructive restore the API then booted from. **Backend 328 tests, web 65.** Fixed one latent defect the first production-only install exposed: `exceljs` was in `devDependencies` despite backing `GET /invoices/export`. Still outstanding: nothing runs on `linuc` yet, `durga`'s nginx/DNS/TLS is untouched (`deploy/nginx/ksef-tenant.conf.template` is committed source only), no amd64 build has been proven (local verification was arm64), and no volume has been seeded with real tenant data. Deployment context for agents lives in [`DEPLOYMENT_CONTEXT.md`](./DEPLOYMENT_CONTEXT.md); the operator guide is [`../docs/DEPLOYMENT.md`](../docs/DEPLOYMENT.md).
