...
**Last updated:** 2026-09-13 20:06 CEST
...
- **[`SALES_INVOICES_PLAN.md`](./SALES_INVOICES_PLAN.md)** — in progress (opened 2026-08-23). Brings sales invoices (`Subject1`) into scope for revenue reporting, stored in the same `invoices` table behind a `direction` column. Stage 0 (reconnaissance) is complete: the existing `InvoiceRead` token authorises `Subject1`, 39 sales invoices were sampled across both tenants, all parsed by the unmodified parser with zero errors and zero KSeF-number collisions — see [`INVOICE_TYPES_ANALYSIS.md`](./INVOICE_TYPES_ANALYSIS.md) §9. Stage 2 (schema) is also complete: migration `0009` adds `invoices.direction`/`invoices.invoice_kind`/`sync_runs.subject_type` and a `not_applicable` categorization confidence, applied and verified on both live tenant databases (row counts, sums, and the `invoice_items` cascade canary all unchanged; 227 backend tests pass). Stage 2b (`invoice_kind` backfill) is also complete: `pnpm run backfill:invoice-kind` derives it offline from stored `raw_xml`, applied to both live tenants after a dry-run and a `VACUUM INTO` backup (parkowa 249/249, portowa 530/530, zero failures; resulting distribution parkowa VAT 228/KOR 20/ROZ 1, portowa VAT 504/KOR 26). Backend now at 235 tests. Remaining stages (sync, API, UI, historical backfill) are not yet implemented. Sales invoices are never categorized.
- **[`DOCKER_DEPLOYMENT_PLAN.md`](./DOCKER_DEPLOYMENT_PLAN.md)** — implemented, **deployed to `linuc` and seeded with real data (2026-09-13)**; that instance is now authoritative and the developer-machine one should no longer be used for real work. **Public since 2026-09-14** at `https://parkowa.zagwozdki.ovh` / `https://portowa.zagwozdki.ovh` via the `durga` edge, behind an added HTTP basic-auth layer in front of the app's own login (stronger app-level authentication still doesn't exist). Containerises both tenants as isolated Compose stacks (`ksef-parkowa` on port 8081, `ksef-portowa` on 8082, plus a throwaway `ksef-local` on 8091), each with an unpublished Fastify `api` container and an nginx `web` sidecar that serves the built SPA and proxies `/api` to it. Adds a multi-stage `Dockerfile` (targets `api`/`web`), a single parameterised `deploy/docker-compose.yml` driven by per-tenant env files, `deploy/ksef-stack.sh` as the sole operator entry point, backup/restore/smoke scripts, an unauthenticated `GET /health` route, and a `VACUUM INTO` snapshot tool (`pnpm run backup:db`). Verified end to end on a local throwaway stack: health, SPA, login, and an authenticated call through the sidecar, plus a real destructive restore the API then booted from. **Backend 328 tests, web 65.** Fixed one latent defect the first production-only install exposed: `exceljs` was in `devDependencies` despite backing `GET /invoices/export`. Post-deploy verification per tenant: parkowa 493 invoices / 4 534 items / 54 sync runs, portowa 655 / 6 557 / 36 — row counts identical to the source across every table, `sync_state` continuation points byte-identical (so the next sync resumes rather than refetching into a rate-limited endpoint), no migration applied on boot, and real data served through the nginx sidecar. Fixed a second defect during the deploy: `smoke.sh` used `source` on the secrets file, which parsed a real KSeF token's `|` separators as a shell pipeline, aborted the script, and printed part of a live token — and while testing that, found Compose interpolates `env_file` values, so a `$` in a secret is silently truncated. `durga`'s nginx/TLS is now configured for both tenants (manual DNS-01 certs, expiring 2026-12-13, **will not auto-renew** — see `.cognitron/contexts/DEPLOYMENT_CONTEXT.md` §11 for the follow-up needed before then). Still outstanding: no nightly `backup.sh` cron is installed, and the branch is not merged to `main`. Deployment context for agents lives in [`DEPLOYMENT_CONTEXT.md`](../.cognitron/contexts/DEPLOYMENT_CONTEXT.md); the operator guide is [`../docs/DEPLOYMENT.md`](../docs/DEPLOYMENT.md).
